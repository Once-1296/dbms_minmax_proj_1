...
// This would normally be 9 separate files imported here.
// For the prototype, we inline the core form fields matching the DDL.
...
export function WorkflowContainer({ project, onUpdateStatus }: { project: Project, onUpdateStatus: (s: ProjectStatus) => void }) {
  ...
  const handleNext = (nextStatus: ProjectStatus) => {
    setLoading(true);
    setTimeout(() => {
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
              <div>
                ...
                <select className="w-full form-input" defaultValue={project.Coordinator_ID || ""}>
                  <option value="3">Dr. R. D. Sharma (ID: 3)</option>
                  <option value="4">Dr. K. Mehta (ID: 4)</option>
                </select>
              </div>
              <div>
        ...
        return (
           <div className="space-y-6">
            ...
            <div className="p-4 rounded-lg bg-emerald-50 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/50">
              <strong>College proposed fee structure:</strong> ₹ 25,00,000 Total Estimated Cost.
            </div>
      ...
      case 'DIRECTOR_APPROVAL':
        return (
           <div className="space-y-6 border-l-4 border-amber-500 pl-4">
            ...
            <p className="text-sm text-slate-500">College Director must review and explicitly set `Director_Approval = TRUE` in `Budget_Estimations`.</p>
            
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-slate-100 dark:bg-slate-800 rounded-lg">
                <p className="text-xs text-slate-500">Total Project Cost</p>
                <p className="text-xl font-bold">₹ {project.Cost_Of_Work.toLocaleString()}</p>
              </div>
              <div className="p-4 bg-slate-100 dark:bg-slate-800 rounded-lg">
                <p className="text-xs text-slate-500">Institute Share Estimated</p>
                <p className="text-xl font-bold focus:ring-amber-500">₹ {(project.Cost_Of_Work * 0.3).toLocaleString()}</p>
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" defaultValue={project.Cost_Of_Work} className="w-full form-input" />
              </div>
               <div>
                <label className="block text-sm font-medium mb-1">Tax Amount (18% GST)</label>
                <input type="number" defaultValue={project.Cost_Of_Work * 0.18} className="w-full form-input" />
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg text-indigo-900 dark:text-indigo-200">
                ...
                <span className="text-2xl font-bold">₹ {((project.Cost_Of_Work) * 0.7).toLocaleString()}</span>
              </div>
              <div className="p-4 bg-rose-50 dark:bg-rose-900/20 rounded-lg text-rose-900 dark:text-rose-200">
                ...
                <span className="text-2xl font-bold">₹ {((project.Cost_Of_Work) * 0.3).toLocaleString()}</span>
              </div>
            </div>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="py-2"><input type="text" value="Dr. A. P. Kelkar" disabled className="form-input text-xs w-full"/></td>
                ...
                <tr>
                  ...
                  <td className="py-2"><input type="text" value="Prof. R. D. Sharma" disabled className="form-input text-xs w-full"/></td>
                  <td className="py-2"><input type="number" placeholder="Amt" className="form-input text-xs w-full"/></td>
                </tr>
                <tr>
                  <td className="py-2"><select className="form-input text-xs"><option>PDF</option></select></td>
                  <td className="py-2"><input type="text" value="Prof. R. D. Sharma (PDF)" disabled className="form-input text-xs w-full"/></td>
                  <td className="py-2"><input type="number" placeholder="Amt" className="form-input text-xs w-full"/></td>
                </tr>
              </tbody>
            </table>
