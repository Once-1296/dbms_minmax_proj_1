...
export default function OrganizationDashboard() {
  ...
  // Mock Metrics based on client
  const clientProjects = mockProjects.filter(p => p.Client_ID === user.Client_ID);
  
  const metrics = [
    { label: "Ongoing Projects", value: clientProjects.length.toString(), icon: FileText, color: "text-brand-500", bg: "bg-brand-50" },
    { label: "Pending Invoices", value: "₹ 5.0 L", icon: Receipt, color: "text-amber-500", bg: "bg-amber-50" },
    { label: "Completed Audits", value: "0", icon: CheckCircle, color: "text-emerald-500", bg: "bg-emerald-50" },
  ];
    ...
    <>
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 shadow-sm">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          Manage your consultancy requests and track project progress. GSTIN: {user.GSTIN_UIN}
        </p>
      </div>
